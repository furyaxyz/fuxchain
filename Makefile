...
Iavl=v0.14.3
Name=fuxchain
ServerName=fuxchaind
ClientName=fuxchaincli
...
ifeq ($(FURYCMALLOC),tcmalloc)
  ldflags += -extldflags "-ltcmalloc_minimal"
endif

ifeq ($(FURYCMALLOC),jemalloc)
...
fuxchain: check_version
	$(cgo_flags) go install -v $(BUILD_FLAGS) -tags "$(build_tags)" ./cmd/fuxchaind
	$(cgo_flags) go install -v $(BUILD_FLAGS) -tags "$(build_tags)" ./cmd/fuxchaincli
...
cli:
	go install -v $(BUILD_FLAGS) -tags "$(build_tags)" ./cmd/fuxchaincli

server:
	go install -v $(BUILD_FLAGS) -tags "$(build_tags)" ./cmd/fuxchaind
...
build:
ifeq ($(OS),Windows_NT)
	go build $(BUILD_FLAGS) -tags "$(build_tags)" -o build/fuxchaind.exe ./cmd/fuxchaind
	go build $(BUILD_FLAGS) -tags "$(build_tags)" -o build/fuxchaincli.exe ./cmd/fuxchaincli
else
	go build $(BUILD_FLAGS) -tags "$(build_tags)" -o build/fuxchaind ./cmd/fuxchaind
	go build $(BUILD_FLAGS) -tags "$(build_tags)" -o build/fuxchaincli ./cmd/fuxchaincli
endif
...
build-docker-fuxchainnode:
	$(MAKE) -C networks/local

# Run a 4-node testnet locally
localnet-start: localnet-stop
	@if ! [ -f build/node0/fuxchaind/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/fuxchaind:Z fuxchain/node testnet --v 4 -o . --starting-ip-address 192.168.10.2 --keyring-backend=test ; fi
	docker-compose up -d
