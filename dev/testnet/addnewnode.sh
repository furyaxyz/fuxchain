...
if [ -d ${FUCHAIN_NET_CACHE}/node0/fuxchaind ]; then
    seed_addr=$(${BIN_NAME} tendermint show-node-id --home ${FUCHAIN_NET_CACHE}/node0/fuxchaind)@${IP}:${seedp2pport}
fi
...
init() {
    ...
    if [ -d ${FUCHAIN_NET_CACHE}/${NAME} ]; then
        echo "Invalid index!"
        echo "<${FUCHAIN_NET_CACHE}/${NAME}> already exists. Use '-n' to try another index."
    ...
    ${BIN_NAME} init ${NAME} -o --chain-id ${CHAIN_ID} --home ${FUCHAIN_NET_CACHE}/${NAME}/fuxchaind --node-index ${INPUT_INDEX}
}
...
start() {
    ...
    rm ${FUCHAIN_NET_CACHE}/${NAME}/fuxchaind/config/genesis.json
    cp ${FUCHAIN_NET_CACHE}/node0/fuxchaind/config/genesis.json ${FUCHAIN_NET_CACHE}/${NAME}/fuxchaind/config/
...
#     echo "${BIN_NAME} --home ${FUCHAIN_NET_CACHE}/${NAME}/fuxchaind  start --p2p.laddr tcp://${IP}:${p2pport} --p2p.seeds ${seednode} --rpc.laddr tcp://${IP}:${rpcport}"
...
#    LOG_LEVEL=main:info,*:error,state:debug,consensus:debug
    ...
    ${BIN_NAME} start \
    --chain-id ${CHAIN_ID} \
    --home ${FUCHAIN_NET_CACHE}/${NAME}/fuxchaind \
    ...
    --rpc.laddr tcp://${IP}:${rpcport} > ${FUCHAIN_NET_CACHE}/rpc${INPUT_INDEX}.log 2>&1 &
