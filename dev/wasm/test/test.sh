...
echo "0-----------------------"
fuxchaincli tx send captain $(fuxchaincli keys show fred -a) 1000fury --fees 0.001fury -y -b block
...
fuxchaincli tx wasm migrate "$CONTRACT" "$BURNER_CODE_ID" "{\"payout\": \"$DEST_ACCOUNT\"}" --from fred  --fees 0.001fury \
 -b block -y -o json | jq
...
echo "### Query old admin: $(fuxchaincli q wasm contract "$CONTRACT" -o json | jq -r '.contract_info.admin')"
echo "### Update contract"
fuxchaincli tx wasm clear-contract-admin "$CONTRACT" --fees 0.001fury \
