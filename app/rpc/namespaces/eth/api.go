...
func (api *PublicEthereumAPI) GasPrice() *hexutil.Big {
	...
	if appconfig.GetFurycConfig().GetDynamicGpMode() != tmtypes.MinimalGpMode {
		...
		if appconfig.GetFurycConfig().GetDynamicGpCoefficient() > 1 {
			coefficient := big.NewInt(int64(appconfig.GetFurycConfig().GetDynamicGpCoefficient()))
...
func (api *PublicEthereumAPI) EstimateGas(args rpctypes.CallArgs) (hexutil.Uint64, error) {
	...
	gasBuffer := estimatedGas / 100 * config.GetFurycConfig().GetGasLimitBuffer()
