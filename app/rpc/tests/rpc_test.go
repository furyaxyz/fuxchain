...
import (
	...
	cosmos_context "github.com/exfury/fuxchain/libs/cosmos-sdk/client/context"
	...
	"github.com/exfury/fuxchain/app/rpc/types"
	apptesting "github.com/exfury/fuxchain/libs/ibc-go/testing"
...
func (suite *RPCTestSuite) SetupTest() {
	...
	// set fuxchaincli path
	cliDir, err := ioutil.TempDir("", ".fuxchaincli")
	if err != nil {
		panic(err)
	}
	...
	// set fuxchaind path
	serverDir, err := ioutil.TempDir("", ".fuxchaind")
	if err != nil {
		panic(err)
	}
...
func (suite *RPCTestSuite) TestEth_GetBlockByHash() {
	...
	// TODO: fuxchain only supports the block query with txs' hash inside no matter what the second bool argument is.
